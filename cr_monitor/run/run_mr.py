...
ret = m.run_account_mr(account = account)

mr_dtc = MrDTC(position = PositionDTC(client = "bm", username = "bm001"))
mr_dtc.mul_range = np.arange(0.78, 0.8, 0.1)
mr_dtc.num_range = np.arange(50, 52, 5)
# mr_dtc.assumed_coins = ["ETH"]
# ret = mr_dtc.run_assumed_open()
mr_dtc.position.get_now_position()
mr_dtc.run_account_mr(client = "bm", username= "bm001")

mr_dt = MrDT(position = PositionDT())
mr_dt.mul_range = np.arange(0.5, 0.51, 0.1)
mr_dt.num_range = np.arange(50, 52, 5)
mr_dt.assumed_coins = ["ADA", "XRP"]
